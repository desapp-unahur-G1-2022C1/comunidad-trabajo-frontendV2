...
const Home = () => {
  ...
  var API_URL = `https://comunidad-backend-v3.herokuapp.com/ofertas/?pagina=0&limite=6&ordenar=id&idEstado=1`;
  ...
  const primerLlamado = async () => {
    if (llamado === false){
      try{
        var api
        if (grupo == 2){
          api = await fetch(`https://comunidad-backend-v3.herokuapp.com/ofertas/cuit/${datosUsuario.id}?pagina=0&limite=6&ordenar=id&idEstado=1`);
        }
        else{
  ...
  const ofertasAPI = async (e, p) => {
    try {
      ...
      if (grupo == 2){
        ...
        api = await fetch(`https://comunidad-backend-v3.herokuapp.com/ofertas/?pagina=0&limite=6&buscarTitulo=${ofertasValue}&ordenar=id&idEstado=1`);
      }
      const datos = await api.json();
      setPagina(1)
      setListaOfertas(datos.ofertas.rows);
  ...
  const cambiarPagina = async (e, p) => {
    var api
    if (grupo == 2){
      api = await fetch(`https://comunidad-backend-v3.herokuapp.com/ofertas/cuit/${datosUsuario.id}?pagina=${p - 1}&limite=6&ordenar=id&buscarTitulo=${busquedaActual}&idEstado=1`);
    }
    else{
      api = await fetch(`https://comunidad-backend-v3.herokuapp.com/ofertas/?pagina=${p - 1}&limite=6&ordenar=id&buscarTitulo=${busquedaActual}&idEstado=1`);
    }
    const datos = await api.json();
    setListaOfertas(datos.ofertas.rows);
    setPagina(p)
