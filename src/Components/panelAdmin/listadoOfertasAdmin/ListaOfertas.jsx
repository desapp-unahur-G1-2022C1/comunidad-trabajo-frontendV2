...
export default function ListaOfertas({ofertas}) {
  const mandarARevision = async (idOferta) => {
    var data = {
      idEstado: 4
    };
        ...
        <TableBody>
          {ofertas.map((oferta) => (
            ...
            >
              ...
              <TableCell align="left"><Link style={{textDecoration:"none"}} to={`/oferta/${oferta.id}`}><Button variant="contained" color='relaxed'sx={{margin:"0.5rem"}}>VER OFERTA</Button></Link><Button variant="outlined" color='error'sx={{margin:"0.5rem"}} onClick={async ()=> mandarARevision(oferta.id)} >DESHABILITAR</Button></TableCell>
