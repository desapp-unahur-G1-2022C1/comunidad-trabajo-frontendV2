...
const ListadoOfertas = () => {
    ...
    const traerOfertas = async (e, p) => {
        try {
            ...
            const api = await fetch(`https://comunidad-backend-v3.herokuapp.com/ofertas/?pagina=0&limite=6&ordenar=id&idEstado=1&buscarTitulo=${ofertaValue}`);
            const datos = await api.json();
            setPagina(1)
            console.log(datos)
            setOfertas(datos.ofertas.rows)
    ...
    const cambiarPagina = async (e, p) => {
        ...
        const api = await fetch(`https://comunidad-backend-v3.herokuapp.com/ofertas/?pagina=${p - 1}&limite=6&ordenar=id&idEstado=1&buscarTitulo=${busquedaActual}`);
        const datos = await api.json();
        setOfertas(datos.ofertas.rows);
        setPagina(p)
